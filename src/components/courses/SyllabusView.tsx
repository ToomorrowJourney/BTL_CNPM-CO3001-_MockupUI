 ...
 */
const SyllabusLoadingSkeleton: React.FC = () => (
...
const EmptySyllabus: React.FC = () => (
	<div className="text-center py-12 bg-gray-50 rounded-lg border border-gray-200">
		...
		<p className="text-gray-500 mt-2">
			The syllabus for this course has not been uploaded yet.
		</p>
	</div>
);
 ...
 * Displays course syllabus with overview, topics, grading, and resources
 */
const SyllabusView: React.FC<SyllabusViewProps> = ({ syllabus, isLoading }) => {
	...
	const totalGradingWeight = useMemo(
		() =>
	...
	return (
		<div className="space-y-8">
				...
				<div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
					...
					<ul className="space-y-4">
						{syllabus.textbooks.map((book, index) => (
							<li
								key={index}
								className="flex flex-col p-3 bg-gray-50 rounded-md border border-gray-100"
							>
					...
					<ul className="space-y-2 list-disc list-inside text-gray-600">
						...
						syllabus.references.length > 0 ? (
							...
							<li className="text-sm text-gray-400 italic list-none">
								No additional references listed.
							</li>
						)}
					</ul>
				</div>
			...
			{syllabus.syllabusDocuments &&
				syllabus.syllabusDocuments.length > 0 && (
					<section className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
						<h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center">
